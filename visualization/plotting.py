...
def plot_all(
    ...
    fig, axs = plt.subplots(
        len(ensemble_theta_noises), len(ensemble_pos_noises), figsize=(len(ensemble_pos_noises)*3, len(ensemble_theta_noises)*3),
        sharey=True
    )
    for j in range(len(ensemble_theta_noises)):
        for i in range(len(ensemble_pos_noises)):
            # Flocking_50_10_0.01_0.1_0.08
            # experiment particles ensembles obs position theta
            names = [
                f'{experiment}_50_{ensembles}_{noise}_{ensemble_pos_noises[i]}_{ensemble_theta_noises[j]}' for noise in test_observation_noise
            ]
            mls = np.array([max_lengths[n] for n in names]).T
            axs[j][i].set_xlabel('Observation Noise')
            axs[j][i].grid(which='major', axis='y', linestyle='--')
            
            axs[j][i].set_ylim((0, 200))
            axs[j][i].scatter(test_observation_noise, mls[0],)
            # axs[j][i].set_xscale('log')
            ob_axis = f'Angle Noise: {ensemble_theta_noises[j]}'
            axs[j][i].errorbar(test_observation_noise, mls[0],yerr=mls[1], capsize=5, elinewidth=1, fmt='o', c=colors[j], label=ob_axis)
            if j == 0:
                axs[j][i].set_title(f'Position Noise: {ensemble_pos_noises[i]}')
            if i == 0:
                axs[j][i].set_ylabel('Tracking Consistency (steps)')
                axs[j][i].legend()
    # plt.title(f'Ensembles: {ensembles}')
    plt.tight_layout()
    plt.savefig(f'plots/grid_seach_{ensembles}.jpg')
    plt.show()
